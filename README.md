...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `QUEUE_CONCURRENCY` | 2 | 翻译任务并发数（越高越快但成本越高） |
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `TRANSLATION_SUMMARY_ENABLED` | false | 是否先生成全局摘要来提升翻译上下文 |
| `TRANSLATION_SUMMARY_MAX_TOKENS` | 200 | 摘要最大输出 tokens |
| `TRANSLATION_SUMMARY_CHUNK_CHARS` | 12000 | 单次摘要输入最大字符数（超出会分块） |
| `TRANSLATION_GLOSSARY_ENABLED` | false | 是否提取术语表以保持翻译一致性 |
| `TRANSLATION_GLOSSARY_MAX_TOKENS` | 200 | 术语表最大输出 tokens |
| `TRANSLATION_GLOSSARY_CHUNK_CHARS` | 12000 | 单次术语表输入最大字符数（超出会分块） |
| `TRANSLATION_CONTEXT_BATCH_SIZE` | 12 | 单批翻译段落数 |
| `TRANSLATION_CONTEXT_PRECEDING_LINES` | 2 | 前文上下文行数 |
| `TRANSLATION_CONTEXT_FOLLOWING_LINES` | 1 | 后文预览行数 |
| `TRANSLATION_CONTEXT_CONCURRENCY` | 2 | 上下文翻译批次并发数（越高越快但更易触发限流） |
| `TRANSLATION_CONTEXT_BATCH_RETRIES` | 0 | 上下文翻译批次重试次数（失败后会优先拆分批次） |
| `TRANSLATION_CONTEXT_MAX_TOKENS` | 500 | 单批上下文翻译最大输出 tokens |
...
| `SEGMENT_MAX_CHARS` | 160 | 原始分段的最大字符阈值 |
| `SEGMENT_MAX_WORDS` | 30 | 原始分段的最大词数阈值 |
| `SUBTITLE_SOURCE_TARGET_MIN_CJK` | 15 | CJK 源字幕优化目标最小长度 |
| `SUBTITLE_SOURCE_TARGET_MAX_CJK` | 25 | CJK 源字幕优化目标最大长度 |
| `SUBTITLE_SOURCE_TARGET_MIN_WORDS` | 11 | 非 CJK 源字幕优化目标最小词数 |
| `SUBTITLE_SOURCE_TARGET_MAX_WORDS` | 20 | 非 CJK 源字幕优化目标最大词数 |
| `SUBTITLE_BILINGUAL_MAX_CHARS_CJK` | 22 | CJK 双语单行最大字符数，超出会尝试拆 cue |
| `SUBTITLE_BILINGUAL_MAX_WORDS` | 14 | 非 CJK 双语单行最大词数，超出会尝试拆 cue |
...
# 性能配置（推荐）
QUEUE_CONCURRENCY=2           # 任务级并发，建议按模型限流能力逐步调高
...
TRANSLATION_GLOSSARY_MAX_TOKENS=200
TRANSLATION_GLOSSARY_CHUNK_CHARS=12000
TRANSLATION_CONTEXT_BATCH_SIZE=12
TRANSLATION_CONTEXT_PRECEDING_LINES=2
TRANSLATION_CONTEXT_FOLLOWING_LINES=1
TRANSLATION_CONTEXT_CONCURRENCY=2
TRANSLATION_CONTEXT_BATCH_RETRIES=0
TRANSLATION_CONTEXT_MAX_TOKENS=500
SEGMENT_MAX_CHARS=160
SEGMENT_MAX_WORDS=30
SUBTITLE_SOURCE_TARGET_MIN_CJK=15
SUBTITLE_SOURCE_TARGET_MAX_CJK=25
SUBTITLE_SOURCE_TARGET_MIN_WORDS=11
SUBTITLE_SOURCE_TARGET_MAX_WORDS=20
SUBTITLE_BILINGUAL_MAX_CHARS_CJK=22
SUBTITLE_BILINGUAL_MAX_WORDS=14
